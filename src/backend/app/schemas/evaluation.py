...
class PhotoFeatures(BaseModel):
    ...
    composition_score: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Rule-of-thirds / leading-lines adherence"
    )
    lighting_quality: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Exposure balance, no blow-out or crush"
    )
...
class EvaluationResponse(BaseModel):
    ...
    preset_recommendation: str | None = None  # best preset for this photo
    description: str = ""
